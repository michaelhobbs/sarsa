...
outputDirectory = 'output-5_static-epsilon-0-5/'
xAxisSteps = 20
yAxisSteps = 20
...
def calculatePlaceCellActivity(ratCoords, placeCellCoords): # TODO: vectorize to use np.exp and no for loop
	sigma = 0.05
...
def updateEligibilityTrace(currentCoords, direction, eligibilityTrace):
	...
	updated[getCellIndex(currentCoords), direction] += 1 # TODO: we can use r(s) instead of 1
	return updated
...
def runSingleTimeStep(currentPosition, currentDirection, weights, eligibilityTrace, epsilon):
	...
	anticipatedOutputNeuronActivityInNextStep = calculateOutputNeuronActivityForDirection(newPosition, nextDirection, weights) # np.amax(calculateOutputNeuronActivity(newPosition, weights))
	# calculate delta and update weights
    # delta = r - Q(s,a) + gamma*Q(s',a')
	delta = reward - calculateOutputNeuronActivityForDirection(currentPosition, currentDirection, weights) + rewardDiscountRate * anticipatedOutputNeuronActivityInNextStep
	weights = updateWeights(delta, weights, eligibilityTrace)
	eligibilityTrace = decayEligibilityTrace(eligibilityTrace)
	newDirection = selectDirection(epsilon, newPosition, weights) # next action a'
...
def runTrial(weights, eligibilityTrace, epsilon, trialNumber, experimentNumber):
	...
	while (currentStep < maxSteps and not hasGoal):
		[ratCoords, currentDirection, reward, hasGoal, weights, eligibilityTrace] = runSingleTimeStep(ratCoords, currentDirection, weights, eligibilityTrace, epsilon) # then run again on output of this function (s', a')
		ratCoordsHistory = np.append(ratCoordsHistory, ratCoords)
		directionHistory = np.append(directionHistory, currentDirection)
		trialReward += reward
		currentStep += 1
	...
	save(ratCoordsHistory, 'ratCoords_' + str(trialNumber), experimentNumber, 'ratCoords')
	save(directionHistory, 'directions_' + str(trialNumber), experimentNumber, 'directions')
	return [weights, eligibilityTrace, hasGoal, currentStep, trialReward, ratCoords]

def plotQuiverWeights(weights, trial, experimentNumber):
	axisSize = xAxisSteps
	xDelta = 1 / (axisSize - 1)
	...
	ax.set_aspect('equal')
	plt.savefig(outputDirectory + str(experimentNumber) + '/plots/trial_'+str(trial)+'.png')
	plt.close()
	...
	plt.savefig(outputDirectory + str(experimentNumber) + '/plots/trial_'+str(trial)+'_norm.png')
	plt.close()
	...
	circle1 = plt.Circle((0.8, 0.8), 0.1, color='r', fill=False)
	plt.gcf().gca().add_artist(circle1)
	plt.savefig(outputDirectory + str(experimentNumber) + '/plots/trial_'+str(trial)+'_full_norm.png')
	plt.close()

def save(data, fileName, experimentNumber, directory):
	np.savetxt(outputDirectory + str(experimentNumber) + '/' + directory + '/' + str(fileName) + '.csv', data, delimiter=",")

def runExperiment(experimentNumber):
	os.makedirs(outputDirectory + str(experimentNumber) + '/plots', exist_ok = True) 
	os.makedirs(outputDirectory + str(experimentNumber) + '/weights', exist_ok = True) 
	os.makedirs(outputDirectory + str(experimentNumber) + '/eligibilityTrace', exist_ok = True) 
	os.makedirs(outputDirectory + str(experimentNumber) + '/ratCoords', exist_ok = True) 
	os.makedirs(outputDirectory + str(experimentNumber) + '/directions', exist_ok = True) 
	maxTrials = 50
	currentTrial = 0
	epsilon = 0.5
	#epsilonLinearDecay = (0.5 - 0.01) / 50
	weights = 0.001 * np.array(np.random.rand(xAxisSteps * yAxisSteps, numDirections))
	plotQuiverWeights(weights, -1, experimentNumber)
	...
	while (currentTrial < maxTrials):
		...
		[weights, eligibilityTrace, hasGoal, currentStep, trialReward, ratCoords] = runTrial(weights, eligibilityTrace, epsilon, currentTrial, experimentNumber)
		...
		plotQuiverWeights(weights, currentTrial, experimentNumber)
		save(weights, 'weights_' + str(currentTrial), experimentNumber, 'weights')
		save(eligibilityTrace, 'eligibilityTrace_' + str(currentTrial), experimentNumber, 'eligibilityTrace')
	...
	a = np.asarray([ trialGoalHistory, trialDurationHistory, trialRewardHistory])
	np.savetxt(outputDirectory + str(experimentNumber) + "/testTrial.csv", a, delimiter=",")
